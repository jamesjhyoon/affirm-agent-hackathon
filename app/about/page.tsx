    ...
    <div className="min-h-screen bg-white text-[#0A2540]">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-12">
        ...
        <Section
          ...
          body={[
            "Reschedule is a servicing-side accommodation, not a contract modification. The Truth in Lending disclosure includes the original payment schedule, but the loan agreement also reserves the servicer's right to grant payment deferrals within policy. No rate change, no term extension, no APR recalculation, no re-papering — the prototype just moves WHEN a single installment is collected, the same shape Affirm itself already supports.",
            "Affirm's production system already has this capability. The codebase exposes user_impresses_deferral_flow / user_interacts_user_impresses_deferral_flow analytics events (packages/analytics/src/generated/codegenEventTypes.ts) and ships mock fixtures rendering 'Payment Deferral: Apr 20, 2020' in the loan timeline. Today the flow lives inside the repayment WebView at products/repayment; the prototype surfaces the same servicing primitive in chat with deterministic eligibility, so a denial returns a one-turn 'no, but here are eligible alternatives' instead of a multi-step webview that lands on a dead end.",
            "Reschedule denials are a SERVICING POLICY result, not a credit decision. \"You've already used your one reschedule this upcoming payment\" is the same shape as \"you've already used your free transfer this month\" on a deposit account — operational limit, not adverse action under Reg B / ECOA.",
            "The 1-per-cycle / 14-day policy is hardcoded in the demo for determinism. In production it would be sourced from Affirm's existing servicing rules engine — same numbers the existing deferral flow uses. The agent doesn't invent the policy; it surfaces a policy result.",
