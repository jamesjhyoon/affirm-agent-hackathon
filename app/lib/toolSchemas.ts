...
export const TOOL_DEFINITIONS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Cross-plan optimizer for hypothetical extra cash. Use when the user describes having an EXTRA dollar amount to allocate ('I have an extra $500', 'I just got a bonus, where should it go', 'where should I put $200', 'what should I pay off first', 'how should I use this windfall', 'I got a tax refund of $1500'). Returns up to four goal-framed options ranked by strategy: (a) when the budget can fully close 2+ plans, an allocate-across option that splits the cash to close multiple plans + applies leftover to the highest-APR plan still standing — this leads when present, (b) save the most on interest (highest-APR plan), (c) clear a plan today (smallest-balance plan that fits), (d) free near-term cash (soonest-due plan). Each option shows the dollar impact + a rationale so the user can pick by goal, not by guessing. The Manage tab cannot do this — it shows balances and APRs separately but cannot rank allocation by intent. Always call this for extra-cash intents BEFORE recommending a single plan; never reason across plans yourself.",
    input_schema: {
      type: "object",
      properties: {
        amount_usd: {
